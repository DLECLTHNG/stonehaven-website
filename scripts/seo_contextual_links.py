...
def reading_paths(pages, local, es):
    ...
    if local in ['/heloc', '/heloc-planning-tools', '/blog/heloc-preparation-checklist']:
        paragraphs.append(('Antes de comprometer el dinero, revise '+a('/blog/heloc-closing-timeline-cancellation-period', 'when HELOC funds become usable', 'cuándo puede usar los fondos del HELOC')+', '+a('/blog/heloc-freeze-credit-line-reduction-plan', 'what a restricted credit line means for a project', 'cómo afecta una restricción de la línea a su proyecto')+' y '+a('/blog/refinance-first-mortgage-keep-heloc-subordination', 'keeping a HELOC when refinancing the first mortgage', 'cómo conservar el HELOC al refinanciar la primera hipoteca')+'.') if es else
                          'Before committing the money, review '+a('/blog/heloc-closing-timeline-cancellation-period', 'when HELOC funds become usable')+', '+a('/blog/heloc-freeze-credit-line-reduction-plan', 'what a restricted credit line means for a project')+' and '+a('/blog/refinance-first-mortgage-keep-heloc-subordination', 'keeping a HELOC when refinancing the first mortgage')+'.')
    if local in ['/dscr', '/resources/first-dscr-loan', '/blog/dscr-loans-explained']:
        paragraphs.append(('Complete el presupuesto con nuestra guía de '+a('/blog/dscr-loan-reserves-cash-to-close', 'DSCR reserves and cash to close', 'reservas DSCR y efectivo al cierre')+'. Compare también las '+a('/blog/dscr-prepayment-penalty-exit-planning', 'DSCR prepayment terms', 'condiciones de prepago DSCR')+' con la fecha prevista de venta o refinanciamiento.') if es else
                          'Complete the cash plan with our guide to '+a('/blog/dscr-loan-reserves-cash-to-close', 'DSCR reserves and cash to close')+'. Compare '+a('/blog/dscr-prepayment-penalty-exit-planning', 'DSCR prepayment terms')+' with your intended sale or refinance date, too.')
    if local in ['/commercial/construction-loans', '/resources/first-rehab-loan']:
        paragraphs.append(('Separe '+a('/blog/construction-loan-interest-reserve-vs-contingency', 'interest reserves, construction contingency and working cash', 'reserva de intereses, contingencia de obra y efectivo operativo')+' antes de comparar el compromiso total del préstamo con el efectivo necesario durante la obra.') if es else
                          'Separate '+a('/blog/construction-loan-interest-reserve-vs-contingency', 'interest reserves, construction contingency and working cash')+' before comparing the total loan commitment with the cash needed during construction.')
    if local in ['/commercial/land-development-loans', '/commercial/development-financing-for-architects']:
        paragraphs.append(('Nuestra guía de '+a('/blog/subdivision-loan-lot-release-prices', 'subdivision lot release prices', 'precios de liberación de lotes')+' explica cómo la venta de un lote se reparte entre reducción de deuda, costos y efectivo disponible para el proyecto.') if es else
                          'Our guide to '+a('/blog/subdivision-loan-lot-release-prices', 'subdivision lot release prices')+' explains how a lot sale is divided between debt reduction, costs and cash remaining for the project.')
    if local in ['/commercial/bridge-loans', '/resources/commercial-refinance-guide']:
        paragraphs.append(('Pruebe el '+a('/blog/commercial-refinance-cash-in-gap', 'commercial refinance cash-in gap', 'faltante de efectivo en un refinanciamiento comercial')+' antes del vencimiento: compare el nuevo préstamo con la liquidación, los costos y las reservas requeridas.') if es else
                          'Test the '+a('/blog/commercial-refinance-cash-in-gap', 'commercial refinance cash-in gap')+' before maturity by comparing replacement proceeds with the payoff, transaction costs and required reserves.')
    if local in ['/commercial/fix-and-flip', '/resources/first-fix-and-flip-financing']:
        paragraphs.append(('Prepare el '+a('/blog/fix-and-flip-rehab-draw-cash-flow', 'rehab draw cash-flow plan', 'plan de efectivo entre desembolsos de rehabilitación')+' y evalúe por separado una '+a('/blog/fix-and-flip-rental-exit-refinance', 'flip-to-rental refinance exit', 'salida de refinanciamiento para conservar el inmueble en alquiler')+' si la venta se retrasa.') if es else
                          'Prepare a '+a('/blog/fix-and-flip-rehab-draw-cash-flow', 'rehab draw cash-flow plan')+' and separately evaluate a '+a('/blog/fix-and-flip-rental-exit-refinance', 'flip-to-rental refinance exit')+' if the sale is delayed.')
