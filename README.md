...
Public contact details are in `js/site-config.js`. Leads use Netlify Forms so existing office email alerts continue. A signed Netlify event forwards supported commercial, SBA and DSCR inquiries to the CRM. Residential, HELOC and Family inquiries remain in Netlify and email. See `docs/WEBSITE-CRM-RELAY.md`. HELOC forms share `js/heloc-fields.js`; all three financial estimates are required, and comparison forms enable them when HELOC is selected.
